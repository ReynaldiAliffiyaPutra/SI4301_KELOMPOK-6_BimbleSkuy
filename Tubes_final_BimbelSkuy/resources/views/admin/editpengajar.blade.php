...
@section('body')
  <div class="container mt-4 mb-4 pt-4">
    ...
    <div style="padding: 2rem; background-color: white">
      <form action="/admin/pengajar/update/{{ $pengajar->id_pengajar }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-3" style="background-color: #C4C4C4; padding: 2rem; width: max-content;">
          <div class="mb-3"
            ...
            <img src="/upload/pengajar/{{ $pengajar->profile }}" alt="" width="100%" style="object-fit: cover;" id="output">
          </div>
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" name="nama" id="nama" value="{{ $pengajar->nama }}" required>
        </div>

        <div class="mb-3">
          <label for="pendidikan" class="form-label">Pendidikan</label>
          <input type="text" class="form-control" name="pendidikan" id="pendidikan" value="{{ $pengajar->tamatan }}" required>
        </div>

        <div class="mb-3">
          <label for="umur" class="form-label">Umur</label>
          <input type="number" class="form-control" name="umur" id="umur" value="{{ $pengajar->umur }}" required>
        </div>

        <div class="mb-3">
          <label for="keahlian" class="form-label">Keahlian</label>
          <input type="text" class="form-control" name="keahlian" id="keahlian" value="{{ $pengajar->keahlian }}" required>
        </div>
